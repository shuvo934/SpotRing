...
// Keep the CPU awake
        ...
        if (geofencingEvent != null) {
            ...
            checkValidation(geoIDs, geofenceTransition,context);

        }
    }

    public void checkValidation(ArrayList<String> geo_ids, int transition, Context context) {
        String url = "https://script.google.com/macros/s/AKfycbzsv_lF5eGCTGmrLjKplFKQkXH9CBMLZl6aNDgvSpRIKk_ezMoDkcPre--M3qpSU_U/exec";
        SharedPreferences sharedPreferences = context.getSharedPreferences(LOGIN_ACTIVITY_FILE, MODE_PRIVATE);
        String token = sharedPreferences.getString(KEY_USER_P_TOKEN, null);
        String expMs = sharedPreferences.getString(KEY_USER_SUB_EXPIRE_M_SECONDS, null);

        boolean checkDateFromServer = isCheckDateFromServer(expMs);
        if (token != null ) {
            if (!token.isEmpty()) {
                ...
                StringRequest request = new StringRequest(
                            ...
                            try {
                                ...
                                if (ok) {
                                    ...
                                    if (expiryMillis > 0) {
                                        ...
                                        getGeoData(geo_ids, transition,context, subOk);
                                    }
                                    else {
                                        getGeoData(geo_ids, transition,context, checkDateFromServer);
                                    }
                                }
                                else {
                                    getGeoData(geo_ids, transition,context, checkDateFromServer);
                        ...
                        error -> getGeoData(geo_ids, transition,context, checkDateFromServer)
                )
                {
                    @Override
    ...
    private static boolean isCheckDateFromServer(String expMs) {
        boolean checkDateFromServer;
        if (expMs != null) {
            if (!expMs.isEmpty()) {
                Date n_date = Calendar.getInstance().getTime();
                long expTime = Long.parseLong(expMs);
                Date expDate = new Date(expTime);
                checkDateFromServer = n_date.getTime() < expDate.getTime();
            } else {
                checkDateFromServer = false;
            }
        } else {
            checkDateFromServer = false;
        }
        return checkDateFromServer;
    }

    public void getGeoData(ArrayList<String> geo_ids, int transition, Context context, boolean subCheck) {
            ...
            database.collection(KEY_LOC_TABLE_NAME).whereEqualTo(KEY_LOC_ID,id)
                    ...
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful() && task.getResult() != null && !task.getResult().getDocuments().isEmpty()) {
                            ...
                            Boolean isFree = documentSnapshot.getBoolean(KEY_GEO_IS_FREE);
                            if (trigger_id != null) {
                                                            ...
                                                            .addOnCompleteListener(task1 -> {
                                                                if (task1.getResult() != null && !task1.getResult().getDocuments().isEmpty()) {
                                                                    ...
                                                                    for (int j = 0; j < documentSnapshots.size(); j++) {
                                                                        ...
                                                                        String wd = documentSnapshots.get(j).getString(KEY_GEO_REPEAT_WEEK_DAY);
    ...
    public void setAlert(String geo_id, String event_name, String notes, String event_type, String trigger_id, String alarm_id, String repeat_id, int transition, Context context, ArrayList<CustomRepetitionDataList> customRepetitionDataLists) {
        boolean geoEnterTrig = trigger_id.equals("1") || trigger_id.equals("4") || trigger_id.equals("5") || trigger_id.equals("7");
        boolean geoExitTrig = trigger_id.equals("2") || trigger_id.equals("4") || trigger_id.equals("6") || trigger_id.equals("7");
        boolean geoDwellTrig = trigger_id.equals("3") || trigger_id.equals("5") || trigger_id.equals("6") || trigger_id.equals("7");
        switch (repeat_id) {
            case "3":
                if (!customRepetitionDataLists.isEmpty()) {
                    ...
                    for (int i = 0; i < customRepetitionDataLists.size(); i++) {
                        ...
                        String w_d = customRepetitionDataLists.get(i).getWeeklyDays();
                        ...
                        if (!b_d.isEmpty() && !e_d.isEmpty()) {
                                            ...
                                            if (date_time.equals(b_time) || date_time.equals(e_time) || (date_time.after(b_time) && date_time.before(e_time))) {
                                                ...
                                                switch (transition) {
                                                    case Geofence.GEOFENCE_TRANSITION_ENTER:
                                                        if (geoEnterTrig) {
                                                            onlyNotification(context, "You have entered your Area for: " + event_name, notes, event_type);
                                                    ...
                                                    case Geofence.GEOFENCE_TRANSITION_EXIT:
                                                        if (geoExitTrig) {
                                                            onlyNotification(context, "You have exited your Area for: " + event_name, notes, event_type);
                                                            if (!alarm_id.equals("1")) {
                                                                Intent serviceIntent = new Intent(context, GeoFenceService.class);
                                                                serviceIntent.putExtra("EV_NA", event_name);
                                                                serviceIntent.putExtra("EV_NO", notes);
                                                    ...
                                                    case Geofence.GEOFENCE_TRANSITION_DWELL:
                                                        if (geoDwellTrig) {
                                                            onlyNotification(context, "You are in your Area for: " + event_name, notes, event_type);
                                                            if (!alarm_id.equals("1")) {
                                                                Intent serviceIntent = new Intent(context, GeoFenceService.class);
                                                                serviceIntent.putExtra("EV_NA", event_name);
                                                                serviceIntent.putExtra("EV_NO", notes);
                            ...
                            String[] daysArray = w_d.split(",");
                            ArrayList<String> daysList = new ArrayList<>(Arrays.asList(daysArray));

                            SimpleDateFormat sdf = new SimpleDateFormat("EEEE", Locale.ENGLISH);
                            String todayName = sdf.format(calendar.getTime());

                            boolean found = false;
                            for (int j = 0; j < daysList.size(); j++) {
                                if (daysList.get(j).equalsIgnoreCase(todayName)) {
                                    found = true;
                                    break;
                                }
                            }
...
//                    context.startActivity(alarmIntent);
    ...
    @SuppressLint("MissingPermission")
    public void onlyNotification(Context context, String contentTitle, String notes, String event_type) {
